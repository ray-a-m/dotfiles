...
- **`bin/`** — system-wide helper scripts copied to `/usr/local/bin/` by `install.sh`. Currently: `vikunja-backup` (driven by the `vikunja-backup.timer` systemd unit; pushes daily SQL+CSV snapshots of the local Vikunja instance to a private GitHub repo for laptop-loss preservation).
