...
- **`bin/`** — system-wide helper scripts copied to `/usr/local/bin/` by `install.sh`. Currently: `tasks-backup` (driven by the `tasks-backup.timer` user-level systemd unit; nightly `git push` of `~/tasks/` to private `ray-a-m/tasks` for laptop-loss preservation).
- **`systemd/`** — Linux systemd units. `system/` units land in `/etc/systemd/system/` (root-owned); `user/` units land in `~/.config/systemd/user/` and run under the per-user systemd manager. Both copied (not symlinked) by `install.sh` since the system loader rejects symlinks resolving into `/home/`.
