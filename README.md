...
- **`bin/`** — system-wide helper scripts copied to `/usr/local/bin/` by `install.sh`. (Currently empty; previously held `tasks-backup`, retired 2026-05-12 when notes/tasks moved to Dropbox.)
- **`systemd/`** — Linux systemd units. `system/` units land in `/etc/systemd/system/` (root-owned); `user/` units (if any) land in `~/.config/systemd/user/` and run under the per-user systemd manager. Both copied (not symlinked) by `install.sh` since the system loader rejects symlinks resolving into `/home/`.
