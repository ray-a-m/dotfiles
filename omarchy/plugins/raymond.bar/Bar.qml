...
Item {
  ...
  // Pill fork: these three were `required`, which only works for the
  // built-in bar (instantiated declaratively with bindings). A cloned bar
  // loads through pluginBarLoader, which assigns properties after creation
  // — `required` aborts creation before that assignment can happen.
  ...
  component BarPanel: PanelWindow {
    ...
    // Pill fork: the horizontal bar never paints a continuous strip — each
    // section draws its own pill surface (see horizontalBar). The vertical
    // bar keeps the stock strip.
    ...
    Component {
      ...
      Item {
        ...
        // Pill fork: translucent rounded surface hugging each section's
        ...
        SectionPill {
          visible: rightMods.width > 0
          anchors.fill: rightMods
          anchors.leftMargin: -Style.space(3)
          anchors.rightMargin: -Style.space(3)
        }
  ...
  // Pill fork: the translucent rounded surface behind each bar section.
  // Same recipe as the pre-Quattro GlassPill primitive: background tint,
  // faint foreground border, full-round ends.
  component SectionPill: BorderSurface {
    z: -1
    color: root.transparent ? "transparent"
      : Qt.rgba(root.background.r, root.background.g, root.background.b, 0.35)
    borderSpec: Border.flat(Qt.rgba(root.barForeground.r, root.barForeground.g, root.barForeground.b, 0.25), 1)
    radius: Math.min(Style.cornerRadius, height / 2)
  }
  ...
  component CenterModules: Item {
    ...
    Component {
      ...
      Item {
        ...
        // Pill fork: one pill spanning the whole center cluster (solo list,
        // or before + anchor + after when a center anchor is set).
        SectionPill {
          readonly property real edgeLeft: centerRoot.hasAnchor
            ? (beforeList.width > 0 ? beforeList.x : centerAnchorModule.x)
            : soloList.x
          readonly property real edgeRight: centerRoot.hasAnchor
            ? (afterList.width > 0 ? afterList.x + afterList.width : centerAnchorModule.x + centerAnchorModule.width)
            : soloList.x + soloList.width

          visible: edgeRight > edgeLeft
          x: edgeLeft - Style.space(3)
          width: (edgeRight - edgeLeft) + Style.space(6)
          height: centerRoot.hasAnchor ? centerAnchorModule.height : soloList.height
          anchors.verticalCenter: parent.verticalCenter
        }
