...
Item {
  ...
  property int cardHeight: root.dmenuActive
    ...
    : Math.min(contentMargin * 2 + (typingMenu ? headerHeight + contentSpacing : 0) + visibleRowsHeight, panel.height - Style.gapsOut * 2)
  ...
  function availableRowsHeight() {
    ...
    var available = panel.height - top - Style.gapsOut - root.contentMargin * 2 - (root.typingMenu ? root.headerHeight + root.contentSpacing : 0)
    // The frozen ceiling only pins the card while a search resizes it; a
  ...
  PanelWindow {
    ...
    // Clone patch: every card opens with its top on the same fixed line
    // (the root menu's old centered position) instead of re-centering per
    // card height, so the apps list and the root menu align.
    readonly property int centeredTop: Math.max(Style.gapsOut, Math.round(height * 0.366))
    ...
    BorderSurface {
      ...
      Item {
        ...
        Keys.onPressed: function(event) {
          ...
          if (event.key === Qt.Key_Delete) {
            ...
            if (event.text === "j") root.select(1)
            else if (event.text === "k") root.select(-1)
            ...
            event.accepted = true
          } else if (root.typingMenu && event.text && event.text.length === 1 && event.text.charCodeAt(0) >= 32 && event.text.charCodeAt(0) !== 127 && (event.modifiers === Qt.NoModifier || event.modifiers === Qt.ShiftModifier)) {
      ...
      Column {
        ...
        Rectangle {
          ...
          radius: root.cornerRadius
          color: "transparent"
          // Clone patch: static menus have no search, so no header row.
          visible: root.typingMenu
