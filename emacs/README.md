...
| `C-c P` | the shell `publish`, from inside — **deploy, not save** (hence a different, capital key): target derived from the buffer (website file → `publish site`, `cv.org` → `cv`, a paper → its slug, dissertation → itself), runs the real shell function async, verdict in the echo area |
| `C-c a` / `C-c l` | Org agenda dispatcher / store-link (capture lives on the splash: `t` task, `n` note; `C-c c` unbound) |
| `C-c M-s` | `org-schedule` (reached as `M-SPC M-s`; mirrors the `M-SPC` family; default `C-c C-s` still works; in the agenda a bare `s` does it, and `M-a M-s` there saves the Org files rather than asking where to write the agenda buffer) — a `SCHEDULED`/`DEADLINE` timestamp is what turns a TODO into a calendar event. Also bound on `C-c M-S` / `C-c S-M-s` for input paths that keep the Shift (this Wayland setup drops it) |
| `C-c G` | push timestamped `inbox.org` TODOs to the Google "org" calendar via **org-gcal** (REST, one-way — nothing read back; visible on the phone + the Omarchy calendar popout, `SUPER+SHIFT+C`). Scheduling a task also auto-pushes just that entry, so `C-c G` is the bulk/manual fallback. Creds live in `~/.authinfo.gpg`; Google 403s CalDAV for unverified apps, hence REST/org-gcal not org-caldav |
